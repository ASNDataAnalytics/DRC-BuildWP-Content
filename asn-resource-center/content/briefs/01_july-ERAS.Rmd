...
description: "July 2018 ERAS application data for the appointment year (AY) 2019 nephrology Match show an uptick in total candidates and applications over the same time point in previous recruitment cycles"  
image: "/images/00_July-Data-Brief_Image.png"
word: "/files/01_July-Data-Brief_Word.docx"
excel: ""
pdf: "/files/01_July-Data-Brief_Word.pdf"
tags: "featured"
...
```{r Load Data, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}
  ...
  eras <- read_excel(here::here("content_data/01_data", 
                                "2018-08-06_ERAS_Legacy-Data_AY-2014_AY-2019.xlsx"), 
                     sheet = "By Edu Status")
...
```{r Figure 1, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}

  eras_colors <- c("2014" = "#cccccc", "2015" = "#cccccc",  "2016" = "#cccccc", 
                    "2017" = "#cccccc",  "2018" = "#cccccc","2019" = "#3366cc")
  
  eras_hist %>% dplyr::filter(month == 7) %>% 
...
```{r Figure 2, echo=FALSE,  message=FALSE, warning=FALSE, cache=TRUE}
  eras_hist %>% dplyr::filter(month == 7) %>% mutate(lagged = lag(`# of Applicants`)) %>% 
    mutate(Pct_Change = ((`# of Applicants` - lagged) / lagged)) %>% 
    dplyr::filter(`ERAS Year` != 2014) %>% 
...
```{r Figure 3, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
  eras_hist %>% dplyr::filter(month == 7) %>% 
...
```{r Figure 4, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
  eras_hist %>% dplyr::filter(month == 7) %>% mutate(lagged = lag(`# of Applications`)) %>% 
...
```{r Figure 5, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}

  
        eras <-  eras %>% mutate(Label = paste0(edu_status, " ", 
                                                as.character(num_candidate)))  
        
        eras %>% dplyr::filter(month == 7) %>% ggplot() +
...
```{r Figure 6, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}

  eras_colors <- c("2014" = "#cccccc", "2015" = "#cccccc",  "2016" = "#cccccc", 
                    "2017" = "#cccccc",  "2018" = "#cccccc","2019" = "#3366cc") 
  
  Avg <- eras_hist %>% dplyr::filter(!`ERAS Year` == 2019) %>% group_by(`ERAS Year`) %>% 
...
```{r Figure 7, echo=FALSE,  message=FALSE, warning=FALSE, cache=TRUE}

  eras_colors <- c("2014" = "#cccccc", "2015" = "#cccccc",  "2016" = "#cccccc", 
                    "2017" = "#cccccc",  "2018" = "#cccccc","2019" = "#3366cc") 
                 
  Avg_Apps <- eras_hist %>% dplyr::filter(!`ERAS Year` == 2019) %>% group_by(`ERAS Year`) %>% 
